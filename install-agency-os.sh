#!/bin/bash
# ============================================
# Agency OS - Script de Instalação Completa
# Execute: bash install-agency-os.sh
# ============================================

echo "╔══════════════════════════════════════════════════════════╗"
echo "║         Agency OS - Instalação Multiagente              ║"
echo "╚══════════════════════════════════════════════════════════╝"
echo ""

# Verifica se está no diretório studio-ia
if [ ! -f "agents.js" ]; then
    echo "❌ Execute este script no diretório do studio-ia"
    echo "   cd ~/studio-ia && bash install-agency-os.sh"
    exit 1
fi
...
# MODULO MEMORY
# ============================================
cat > core/memory/index.js << 'MEMEOF'
'use strict';

/**
 * Core Memory - Memória compartilhada entre agentes.
 * Persiste contexto, histórico e decisões entre os agentes.
 */
...
MEMEOF
echo "✅ core/memory/index.js criado"

# ============================================
# MODULO EVENTS
# ============================================
cat > core/events/index.js << 'EVTEOF'
'use strict';

/**
 * Core Events - Sistema de eventos para Agency OS.
 * Dispara eventos quando ações acontecem e notifica o orquestrador.
 */
...
module.exports = { AgencyEvents, events, novoLead, leadQualificado, propostaCriada, landingPronta, campanhaCriada, clienteFechado, relatorioAtualizado, lerEventosRecentes };
EVTEOF
echo "✅ core/events/index.js criado"

# ============================================
# DEMO - Pipeline Completo
# ============================================
cat > core/demo.js << 'DEMOEOF'
#!/usr/bin/env node
'use strict';
...
async function demoPipeline() {
  ...
  console.log('   workspace/memory/pipeline.json');
  console.log('   workspace/memory/historico.json');
  console.log('   workspace/reports/events/*.json');
}

demoPipeline().catch(console.error);
DEMOEOF
chmod +x core/demo.js
echo "✅ core/demo.js criado"

# ============================================
# ORCHESTRATOR
# ============================================
cat > core/orchestrator/index.js << 'ORCHEOF'
'use strict';
...
async function cicloCEO(meta) {
  ...
  const pipeline = memory.listarPipeline();
  log(`Pipeline: ${pipeline.length} leads`);
...
async function agenteGrowth() {
  ...
  log(`GROWTH: ${leads.length} leads encontrados`);
  return leads;
}
...
async function agenteClientes() {
  ...
  const leads = memory.listarPipeline('novo');
  const propostas = leads.map(l => {
    memory.atualizarPipeline({ id: l.id, status: 'proposta_criada', proximo_agente: 'CEO' });
    memory.adicionarHistorico('Clientes', 'proposta_criada', { leadId: l.id });
    events.propostaCriada({ leadId: l.id });
    return l;
  });
...
async function agenteCriacao() {
  ...
  propostas.forEach(p => {
    memory.atualizarPipeline({ id: p.id || p.leadId, status: 'landing_pronta', proximo_agente: 'Tráfego' });
    memory.adicionarHistorico('Criacao', 'landing_criada', { leadId: p.id || p.leadId });
    events.landingPronta({ leadId: p.id || p.leadId });
  });
  log(`CRIACAO: ${propostas.length} landings criadas`);
  return propostas;
}
...
async function agenteTrafego() {
  ...
  landings.forEach(l => {
    memory.atualizarPipeline({ id: l.id, status: 'campanha_criada', proximo_agente: 'Dados' });
    memory.adicionarHistorico('Trafego', 'campanha_criada', { leadId: l.id });
    events.campanhaCriada({ leadId: l.id });
  });
  log(`TRAFEGO: ${landings.length} campanhas criadas`);
  return landings;
}
...
async function agenteDados() {
  ...
  events.relatorioAtualizado({ tipo: 'pipeline-completo' });
  return memory.listarPipeline();
}

async function executarPipelineCompleto() {
  log('=== INICIANDO PIPELINE COMPLETO ===');
  await cicloCEO('Crescimento de clientes');
  await agenteGrowth();
  await agenteClientes();
  await agenteCriacao();
  await agenteTrafego();
  await agenteDados();
...
module.exports = { cicloCEO, agenteGrowth, agenteClientes, agenteCriacao, agenteTrafego, agenteDados, executarPipelineCompleto, status, log };
ORCHEOF
echo "✅ core/orchestrator/index.js criado"

echo ""
echo "✅═══════════════════════════════════════════════════════════✅"
echo "   Agency OS Core Multiagente - Instalação Completa!"
echo "✅═══════════════════════════════════════════════════════════✅"
echo ""
echo "🚀 Para executar a demo:"
echo "   node core/demo.js"
echo ""
echo "📁 Estrutura criada:"
echo "   studio-ia/core/memory/index.js"
echo "   studio-ia/core/events/index.js"
echo "   studio-ia/core/orchestrator/index.js"
echo "   studio-ia/core/demo.js"
echo ""
echo "   studio-ia/workspace/memory/"
echo "   studio-ia/workspace/reports/events/"
echo ""