...
const PROVIDERS = {
  ...
  meta: {
    ...
    canal: 'instagram',
    kind: 'api',
    descricao: 'DM via Graph API da Meta. Informe App ID + App Secret e use "Conectar com Facebook" — o sistema busca o token da página sozinho. A Meta bloqueia DM frio fora da janela de 24h.',
    fields: [
      { key: 'appId', label: 'App ID (Meta)', placeholder: '1021762344042167' },
      { key: 'appSecret', label: 'App Secret', secret: true },
      { key: 'igId', label: 'IG/Page ID (automático ao conectar)', placeholder: 'preenchido ao conectar' },
      { key: 'pageToken', label: 'Token da página (automático)', secret: true },
    ],
...
class SocialHub {
  ...
  metaOauthUrl(redirectUri, state) {
    const cfg = this.config('meta');
    if (!cfg.appId) return null;
    const scope = ['pages_show_list', 'pages_messaging', 'pages_read_engagement', 'business_management', 'instagram_basic', 'instagram_manage_messages'].join(',');
    const u = new URL('https://www.facebook.com/v21.0/dialog/oauth');
    u.searchParams.set('client_id', cfg.appId);
    u.searchParams.set('redirect_uri', redirectUri);
    u.searchParams.set('scope', scope);
    u.searchParams.set('state', state || 'meta');
    u.searchParams.set('response_type', 'code');
    return u.toString();
  }

  async metaExchange(code, redirectUri) {
    const cfg = this.config('meta');
    if (!cfg.appId || !cfg.appSecret) return { ok: false, error: 'Preencha App ID e App Secret e salve antes de conectar.' };
    const enc = encodeURIComponent;
    const errMsg = (r) => (r.data && r.data.error && r.data.error.message) || `HTTP ${r.status}`;

    let r = await httpsJson('GET', `https://graph.facebook.com/v21.0/oauth/access_token?client_id=${enc(cfg.appId)}&client_secret=${enc(cfg.appSecret)}&redirect_uri=${enc(redirectUri)}&code=${enc(code)}`, {});
    if (r.status !== 200 || !r.data || !r.data.access_token) return { ok: false, error: errMsg(r) };
    let userToken = r.data.access_token;

    r = await httpsJson('GET', `https://graph.facebook.com/v21.0/oauth/access_token?grant_type=fb_exchange_token&client_id=${enc(cfg.appId)}&client_secret=${enc(cfg.appSecret)}&fb_exchange_token=${enc(userToken)}`, {});
    if (r.status === 200 && r.data && r.data.access_token) userToken = r.data.access_token;

    r = await httpsJson('GET', `https://graph.facebook.com/v21.0/me/accounts?fields=id,name,access_token,instagram_business_account&limit=50&access_token=${enc(userToken)}`, {});
    if (r.status !== 200 || !r.data || !Array.isArray(r.data.data)) return { ok: false, error: errMsg(r) };
    const pages = r.data.data;
    if (!pages.length) return { ok: false, error: 'Nenhuma página encontrada nessa conta (crie/escolha uma página e tente de novo).' };
    const page = pages[0];

    const data = this.load();
    data.providers.meta = {
      ...(data.providers.meta || {}),
      igId: (page.instagram_business_account && page.instagram_business_account.id) || page.id,
      pageId: page.id,
      pageNome: page.name,
    ...
    this.save(data);
    return { ok: true, page: page.name, pages: pages.map(p => ({ id: p.id, name: p.name })) };
  }
