...
const { renderStyled, LANDING_STYLES, recommendStyle } = require('./landing-styles.js');
const { renderCrm } = require('./crm-template.js');
const { renderApp } = require('./app-template.js');

const ESTILO_HINT = {
  cinema: 'Estilo escolhido: CINEMA (imersivo/cinematográfico). Copy curta e impactante, headline em 1-2 linhas fortes.',
  editorial: 'Estilo escolhido: EDITORIAL DE LUXO (elegante, revista). Copy sofisticada e sóbria, headline com ar de manchete refinada.',
  bold: 'Estilo escolhido: BOLD (alto contraste, jovem). Copy direta, energética, verbos no imperativo, headline curta e marcante.',
  glass: 'Estilo escolhido: GLASS/AURORA (tech/SaaS). Copy moderna focada em benefício e resultado, tom de produto digital.',
  boutique: 'Estilo escolhido: BOUTIQUE QUENTE (aconchegante, artesanal). Copy calorosa e humana, próxima e acolhedora.',
  corporate: 'Estilo escolhido: CORPORATE CLEAN (confiável, B2B/saúde). Copy clara, profissional e que transmite segurança.',
};
...
function buildProdutoTask(cliente, tipo, opts) {
  ...
  const estiloHint = o.estilo && ESTILO_HINT[o.estilo] ? `\n${ESTILO_HINT[o.estilo]}` : '';

  if (tipo === 'landing') {
    return `Você é diretor de arte e copywriter de conversão. Gere o CONTEÚDO de uma landing page de altíssimo padrão para "${nome}", 100% adaptado ao negócio real abaixo.${tema}${estiloHint}
...
function parseProduto(result, tipo, estilo) {
  ...
  if (tipo === 'landing') {
    ...
    if (data && (data.brand || Array.isArray(data.scenes))) {
      try { const html = renderStyled(estilo, data); if (html && html.length > 200) return { formato: 'html', conteudo: html }; } catch {}
    }
...
module.exports = { PRODUTO_TIPOS, buildProdutoTask, parseProduto, LANDING_STYLES, recommendStyle };
