...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0;}
  ...
  .mnav a{ font-family:'Anton',sans-serif; font-size:clamp(22px,8vw,34px); text-transform:uppercase; padding:8px 0; overflow-wrap:anywhere; }
  ...
  .scene-copy h2{ font-family:'Anton',sans-serif; text-transform:uppercase; line-height:0.98; font-size:clamp(48px,11vw,128px); color:var(--bone); text-shadow:0 3px 0 rgba(0,0,0,0.3), 0 22px 60px rgba(0,0,0,0.7); overflow-wrap:anywhere; max-width:100%; }
  ...
  .hero-title .l1{ display:block; font-size:clamp(52px,14vw,150px); overflow-wrap:anywhere; max-width:100%; } .hero-title .l1 span{ color:var(--gold-lite); }
  .hero-script{ font-family:'Dancing Script',cursive; font-weight:700; font-size:clamp(30px,6vw,64px); color:var(--gold-lite); display:block; margin-top:4px; max-width:100%; overflow-wrap:anywhere; }
  @media (max-height:760px){
    .hero-title .l1{ font-size:clamp(38px,11vw,104px); }
    .hero-script{ font-size:clamp(22px,4.5vw,42px); margin-top:2px; }
    .hero-sub{ margin-top:12px; }
    .rating{ margin-top:8px; }
    .btn-row{ margin-top:16px; gap:10px; }
    .scene-copy .eyebrow{ margin-bottom:10px; }
    .scene-copy h2{ font-size:clamp(32px,9vw,84px); }
    .scene-copy .body{ margin-top:10px; }
  ...
  .svc-top{ display:flex; flex-wrap:wrap; justify-content:space-between; align-items:baseline; gap:6px 14px; }
  .svc-name{ font-family:'Barlow Semi Condensed',sans-serif; font-weight:600; text-transform:uppercase; letter-spacing:0.05em; font-size:18px; min-width:0; overflow-wrap:break-word; }
  .svc-price{ font-family:'Anton',sans-serif; font-size:22px; color:var(--gold-lite); line-height:1; }
  ...
  .visit h2{ font-family:'Anton',sans-serif; text-transform:uppercase; font-size:clamp(44px,9vw,90px); line-height:0.98; margin-top:12px; color:var(--bone); overflow-wrap:anywhere; }
  ...
  .book h2{ font-family:'Anton',sans-serif; text-transform:uppercase; font-size:clamp(40px,8vw,84px); line-height:0.98; margin-top:12px; color:var(--bone); overflow-wrap:anywhere; }
...
<script>
  ...
  function fitLine(el){
    if(!el) return;
    const box = el.closest('.scene-copy') || el.parentElement;
    if(!box) return;
    el.style.fontSize = '';
    let size = parseFloat(getComputedStyle(el).fontSize);
    const avail = box.clientWidth;
    let guard = 0;
    while(el.scrollWidth > avail + 1 && size > 15 && guard < 60){ size -= 1; el.style.fontSize = size + 'px'; guard++; }
  }
  function fitAllHeadlines(){
    document.querySelectorAll('.hero-title .l1, .hero-script, .scene-copy h2, .visit h2, .book h2').forEach(fitLine);
  }
  addEventListener('resize', fitAllHeadlines, { passive:true });
  ...
  requestAnimationFrame(()=>{ render(); updateDots(); fitAllHeadlines(); });
</script>
</body>
</html>`;
}
