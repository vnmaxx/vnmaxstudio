...
const { roteiroToAss, transcriptToAss } = require('./video-ass.js');
...
async function processJob(jobDir, roteiro) {
  ...
  const tmp = [];
  try {
    writeStatus(jobDir, { state: 'running', step: clips.length > 1 ? `normalizando ${clips.length} clipes` : 'preparando vídeo (9:16, áudio)' });
    const norm = [];
    ...
    let assObj = null, legendas = 'roteiro', engineUsed = null;
    const eng = transcribe.available();
    if (eng) {
      writeStatus(jobDir, { state: 'running', step: `transcrevendo a fala (${eng})` });
      try {
        const audio = path.join(jobDir, 'audio.wav');
        await run(bin, ['-y', '-i', base, '-vn', '-ac', '1', '-ar', '16000', '-c:a', 'pcm_s16le', audio], jobDir);
        tmp.push(audio);
        const t = await transcribe.transcribe(audio);
        if (t && (t.words || t.segments)) { assObj = transcriptToAss(t, roteiro || {}); legendas = 'sincronizada'; engineUsed = eng; }
      } catch (e) { engineUsed = null; }
    }
    if (!assObj) assObj = roteiroToAss(roteiro || {});
    fs.writeFileSync(path.join(jobDir, 'subs.ass'), assObj.ass, 'utf8');

    writeStatus(jobDir, { state: 'running', step: 'queimando legendas' });
    const final = path.join(jobDir, 'final.mp4');
    await run(bin, ['-y', '-i', base, '-vf', 'subtitles=subs.ass,format=yuv420p',
      '-c:v', 'libx264', '-crf', '21', '-preset', 'veryfast', '-c:a', 'copy', '-movflags', '+faststart', final], jobDir);

    for (const f of tmp) { try { fs.unlinkSync(f); } catch {} }
    try { for (const c of clips) fs.unlinkSync(c); } catch {}
    let size = 0; try { size = fs.statSync(final).size; } catch {}
    writeStatus(jobDir, { state: 'done', step: 'pronto', final: 'final.mp4', size, legendas, transcricao: engineUsed, finishedAt: new Date().toISOString() });
  } catch (e) {
    for (const f of tmp) { try { fs.unlinkSync(f); } catch {} }
