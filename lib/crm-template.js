...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0;} body{ background:var(--bg); color:var(--bone); font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif; }
  ...
  #sync{ font-size:10px; color:var(--dim); margin-top:2px; }
...
<body>
<header>
  <div class="brand"><b>${esc(brand)}</b><span>CRM${ramo ? ' · ' + esc(ramo) : ''}</span><span id="sync">● local (offline)</span></div>
  <div class="toolbar">
...
<script>
  ...
  function loadLocal(){ try{ const r=localStorage.getItem(KEY); if(r) return JSON.parse(r); }catch(e){} return null; }
  function seed(){ return SEED.map((l,i)=>Object.assign({ id:'l'+Date.now()+i, stage:(l.stage||CFG.stages[0].id) }, l)); }
  function saveLocal(a){ try{ localStorage.setItem(KEY, JSON.stringify(a)); }catch(e){} }
  let leads = loadLocal() || seed(); saveLocal(leads);
  let editId = null;

  let fbReady=false, fbCol=null;
  function fbInit(){
    const F=window.VNMAX_FB; if(!F||fbReady) return; fbReady=true;
    try{
      fbCol=F.collection(F.db, 'crm_'+(F.clienteId||'${slug(brand)}'));
      F.getDocs(fbCol).then(snap=>{ const remote=new Set(); snap.forEach(dd=>remote.add(dd.id)); leads.forEach(l=>{ if(!remote.has(l.id)){ try{ F.setDoc(F.doc(fbCol,l.id),l); }catch(e){} } }); }).catch(()=>{});
      F.onSnapshot(fbCol, snap=>{ const arr=[]; snap.forEach(dd=>arr.push(Object.assign({id:dd.id}, dd.data()))); leads=arr; saveLocal(leads); render(); });
      const s=document.getElementById('sync'); if(s){ s.textContent='● sincronizado (Firebase)'; s.style.color='var(--green)'; }
    }catch(e){}
  }
  function fbPut(l){ if(fbReady&&fbCol){ try{ window.VNMAX_FB.setDoc(window.VNMAX_FB.doc(fbCol,l.id), l); }catch(e){} } }
  function fbDel(id){ if(fbReady&&fbCol){ try{ window.VNMAX_FB.deleteDoc(window.VNMAX_FB.doc(fbCol,id)); }catch(e){} } }
  window.addEventListener('vnmax-fb-ready', fbInit);
  if(window.VNMAX_FB) fbInit();
  ...
  function render(){
    ...
    for(const st of CFG.stages){
      ...
      cards.addEventListener('drop',e=>{e.preventDefault(); col.classList.remove('drag'); const id=e.dataTransfer.getData('id'); const l=leads.find(x=>x.id===id); if(l){ l.stage=st.id; saveLocal(leads); fbPut(l); render(); }});
  ...
  function salvar(){
    ...
    let alvo;
    if(editId){ alvo=leads.find(x=>x.id===editId); Object.assign(alvo,obj); }
    else { obj.id='l'+Date.now(); leads.push(obj); alvo=obj; }
    saveLocal(leads); fbPut(alvo); fechar(); render();
  }
  function excluir(id){ if(!confirm('Excluir este lead?')) return; leads=leads.filter(x=>x.id!==id); saveLocal(leads); fbDel(id); render(); }
