...
class Crm {
  ...
  setRascunho(id, msg) {
    ...
    const lead = data.leads[id];
    if (!lead) return null;
    const opcoes = Array.isArray(msg.opcoes)
      ? msg.opcoes
          .filter(o => o && o.mensagem && String(o.mensagem).trim())
          .slice(0, 3)
          .map(o => ({
            angulo: o.angulo || '',
            assunto: o.assunto || '',
            mensagem: String(o.mensagem || '').slice(0, 4000),
            objetivo: o.objetivo || '',
          }))
      : [];
    const principal = opcoes[0] || msg;
    lead.rascunho = {
      canal: msg.canal || lead.canal || '',
      etapa: msg.etapa || 'abertura',
      stage: msg.stage || lead.stage || '',
      assunto: principal.assunto || '',
      mensagem: String(principal.mensagem || '').slice(0, 4000),
      objetivo: principal.objetivo || '',
      proximo_passo: msg.proximo_passo || '',
      opcoes,
      materialTipo: msg.materialTipo || '',
      materialTitulo: msg.materialTitulo || '',
      materialUrl: msg.materialUrl || '',
      status: 'pendente',
